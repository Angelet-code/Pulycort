...
Create a local `.env` file from `.env.example` and set the PostgreSQL connection string:
...
The first test endpoint for the existing table is:

```bash
GET /partes-disco-puente/:id
```

> El modo demo del backend se retiró (2026-06-13). La demostración vive ahora
> solo en el frontend (switch **Demo | Real**), así que este backend siempre
> sirve datos reales y requiere una `DATABASE_URL` válida. Si arranca con
> `DATABASE_PROVIDER=mock` fallará de forma explícita.

## Fabric API

Endpoints para el contrato del frontend:
...
Provide a real `DATABASE_URL` in the host environment, then build and run:
