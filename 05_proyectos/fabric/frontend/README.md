...
| `/partes/tablas` | **Inventario de tablas**: tablas en Odoo y producción real pendiente de reflejo, con **tres procedencias** — existencias on-hand de Odoo (`stock_lot` `type_product_lot='tables'` vía `stock_quant`), **altas recibidas** (`lot_tables_creation`, "Recepción reciente") y **partes de paquetes de telar** (`parte_trabajo_mapeada.operacion='4'`, "Aserrado pendiente de Odoo") con tablas/m² reales. Nº de lote/PM, material, tipo, ubicación o procedencia, medidas, paquetes, tablas y m². Búsqueda por nº de lote, filtros por material y fechas. El aserrado pendiente no garantiza stock actual depurado; queda por conciliar con salidas posteriores de tablas. |
| `/inventario` | **Mapa de existencias** (treemap tipo WinDirStat): cada material es un rectángulo proporcional a su volumen/superficie por forma — Bloques en m³; Tablas y Losas en m² desde stock/altas y, para Tablas, aserrado pendiente de Odoo. Clic en un material para desglosarlo. |
...
- Golpes: **golpes/min** (~80–90 en marcha; la columna cruda viene ×10 y el
  backend la divide entre 10) · Potencia: **kW** (máx. 76) ·
