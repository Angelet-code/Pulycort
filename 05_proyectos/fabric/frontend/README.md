...
| `/partes/tablas` | **Inventario de tablas**: las tablas que realmente hay en almacén, gemelo del inventario de bloques con sus **dos eras** — existencias on-hand de Odoo (`stock_lot` `type_product_lot='tables'` vía `stock_quant`) y **altas recibidas** (`lot_tables_creation`, marcadas "Recepción reciente") aún sin reflejo en el stock. Nº de lote, material (nombre real de `product_template`, p. ej. TRAVERTINOS/MARFIL/MARRON), tipo, ubicación, medidas, nº de paquetes y de tablas. Búsqueda por nº de lote, filtros por material y fechas. El m² va "—" (derivable de `n_tables` en las altas, pendiente de confirmar); el grueso llega con unidades inconsistentes entre lotes (a confirmar). |
